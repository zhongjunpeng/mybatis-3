...
public class PooledDataSource implements DataSource {
    ...
    // 将使用完的连接，添加回连接池中。
    // 调用时机：在 PooledConnection#invoke() 方法 methodName = close 方法的情况下时被调用。
        ...
        synchronized (state) {
            ...
            if (conn.isValid()) {// 有效
                ...
                if (state.idleConnections.size() < poolMaximumIdleConnections
                        && conn.getConnectionTypeCode() == expectedConnectionTypeCode) { // 没超过空闲连接上限
                    // 统计连接使用时长
                    ...
                    // 设置原连接失效
                    // 这里为什么要创建新的 PooledConnection 对象呢？避免使用方还在使用 这个连接，通过将它设置为失效，万一再次调用，会抛出异常
                    conn.invalidate();
                    if (log.isDebugEnabled()) {
        ...
        while (conn == null) {
            ...
            synchronized (state) {
                ...
                if (!state.idleConnections.isEmpty()) { // 有空闲连接
                    ...
                    // 激活的连接数小于 poolMaximumActiveConnections；活跃连接数是否已经达到最大值
                    if (state.activeConnections.size() < poolMaximumActiveConnections) { // 活跃连接数小于最大值
                        ...
                        if (longestCheckoutTime > poolMaximumCheckoutTime) { // 如果超时
                            /**
                             * 获取的连接已超时，那么就可以重新使用该连接的真实数据库连接了。所以，我们可以发现，连接的超时发现，
                             * 并不是由一个定时任务后台执行，而是有点类似懒加载的方式，在连接不够的时候，
                            ...
                            // 如果不是自动提交的，需要进行回滚。即将原有执行中的事务，全部回滚。
    ...
    protected boolean pingConnection(PooledConnection conn) {
        ...
        if (result) { // 连接没关闭，没有抛出异常，就判断是否能 ping
            // 是否启用侦测查询
            if (poolPingEnabled) {
